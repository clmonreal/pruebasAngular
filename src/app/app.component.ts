...
export class AppComponent implements AfterViewInit, OnInit {
  ...
  ngAfterViewInit() {
    ...
    //   // { time: '2019-04-19', value: 74.43 },
    //   // { time: '2019-04-20', value: 81.89 },
    // ]);

    this.syncToInterval(this.activeItem);
  }


  setActiveItem(item: string): void {
    this.activeItem = item;
    this.syncToInterval(item);
  }

  syncToInterval(interval: string): void {
    if (this.areaSeries) {
      this.chart.removeSeries(this.areaSeries);
    }

    this.areaSeries = this.chart.addAreaSeries({
      topColor: 'rgba(251, 193, 42, 0.56)',
      bottomColor: 'rgba(251, 193, 42, 0.04)',
      lineColor: 'rgba(251, 193, 42, 1)',
    ...
    // Definir los datos para cada intervalo
    let seriesesData: any[] = [];
